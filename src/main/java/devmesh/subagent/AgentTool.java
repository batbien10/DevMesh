...
public class AgentTool implements Tool {
    ...
    private devmesh.worktree.WorktreeManager worktreeManager;
     ...
     */
    private devmesh.toolresult.ContentReplacementState parentReplacementState;

    public void setParentReplacementState(devmesh.toolresult.ContentReplacementState state) {
        this.parentReplacementState = state;
    }

    public void setWorktreeManager(devmesh.worktree.WorktreeManager worktreeManager) {
        this.worktreeManager = worktreeManager;
    }

    public void setTeamManager(devmesh.teams.TeamManager teamManager) {
        ...
        for (var msg : parent.getMessages()) {
            if (msg.getToolUses() != null && !msg.getToolUses().isEmpty()
                ...
                var placeholders = msg.getToolUses().stream()
                        .map(tu -> new devmesh.conversation.ToolResultBlock(
    ...
    private ToolResult runSync(SubAgentSpec spec, String description, String prompt, String modelOverride, String isolation) {
        ...
        if ("worktree".equals(isolation) && worktreeManager != null) {
            ...
            try {
                wtResult = devmesh.worktree.AgentWorktree.create(
                ...
                String notice = devmesh.worktree.AgentWorktree.buildNotice(
        ...
        while (true) {
            ...
            switch (event) {
                ...
                case AgentEvent.LoopComplete lc -> {
                    ...
                    String wtInfo = "";
                    if (wtResult != null) {
                        if (devmesh.worktree.WorktreeChanges.hasChanges(
                            ...
                            devmesh.worktree.AgentWorktree.remove(
    ...
    private ToolResult runAsTeammate(SubAgentSpec spec, String teamName,
        ...
        subRegistry.register(new devmesh.teams.TeamTools.SendMessageTool(teamManager, memberName));
        ...
        // Build addendum
        String addendum = devmesh.teams.TeammateRunner.buildTeammateAddendum(
        ...
        if ("worktree".equals(isolation) && worktreeManager != null) {
            try {
                ...
                new java.security.SecureRandom().nextBytes(rndBytes);
                String slug = "agent-a" + java.util.HexFormat.of().formatHex(rndBytes).substring(0, 7);
                var wtResult = devmesh.worktree.AgentWorktree.create(
                        slug, worktreeManager.getProjectRoot(), worktreeManager.getSymlinkDirs());
        ...
        // Spawn teammate
        try {
            var spawnResult = devmesh.teams.SpawnDispatcher.spawnTeammate(
                    new devmesh.teams.SpawnDispatcher.SpawnConfig(
