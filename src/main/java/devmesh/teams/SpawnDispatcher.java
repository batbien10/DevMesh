...
public final class SpawnDispatcher {
    ...
    public record SpawnConfig(
            ...
            String protocol,
            devmesh.config.ProviderConfig providerConfig,
            String workdir
    ) {}
     ...
     * Format: cd '<workdir>' && '<devmesh>' --teammate --team-name <t> --agent-name <n>
     */
    public static String buildTeammateCLI(String teamName, String memberName, String workdir) {
        ...
        // Find devmesh executable (assume it's the current JAR or on PATH)
        String devmesh = ProcessHandle.current().info().command().orElse("devmesh");
        return "cd %s && %s --teammate --team-name %s --agent-name %s".formatted(
                shellQuote(wd), shellQuote(devmesh), shellQuote(teamName), shellQuote(memberName));
    }
